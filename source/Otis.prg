...
// Dn not include, if leto file function
// are necessary call them by there leto rdd name
//#include "LETOFILE.ch"
...
//          With this method only this prg file has to be added to your project.
//****************************************************************************************************************

// Version info
#define  Version        "V1.20"
#define  Versiondate    "01/03/2021"
...
//
PROCEDURE OTIS( pn,pl,rv )

   // Locals
   LOCAL temp, i, x, y, ;                       // some common vars used everywhere
...
//******************************************************************************
   ...
   Public th_bt_bgcol                           // TEST bg color
   ...
   PUBLIC ini_lOpen_Exclusive := .F.,;          // default is open files in shared mode.
          ...
          ini_default_mem   := "DBFFPT"         // default dbf mem driver (mem or fpt)
   ...
   */
   ...
   IF lStandAlone
      ...
      *
      ...
      // we have clients with files bigger than 1.5GB
      // and if this value is not set random dbfcdx/1010 dos error 33 will occur
      // in a multi-user environment because file locking size is limited to 1GB.
      //                                                 Standard is 1GB
      SET(_SET_DBFLOCKSCHEME, DB_DBFLOCK_HB32 )       // value 4, max 4GB
      ...
      */
      ...
      // no filter optimisation with the help of index files
      //SET OPTIMIZE OFF
   ...
   // set LetoDbf ip server with settings in ini file
   //ini_leto_ipserver := ini_leto_ipserver
   ...
   IF !lStandalone
      ...
      FOR i := ini_area_min_scan TO ini_area_max_scan
         ...
         IF !EMPTY( ALIAS(i) ) .and. i < ini_area_pi_reopen_start
            ...
            Try
               USE (cUseName) ALIAS ( cAlias+'_'+hb_ntos(i))
   ...
   //
      ...
      if !empty(cCmdline)
         ...
         // transform all to ";" before tokenizing
         ...
         for i := 1 to len(aTokens)
            ...
            Try
               ...
               // clear flag Dataset has changed to prevent saving settings on exit.
               // Details, see function otis_release()
               lDsChanged := .F.

            Catch oError
               MsgStop("Otis can not open file : "+ aTokens[i] + crlf + crlf + ErrorMessage(oError) )
            End
   ...
   // fill tbrowse dataset array with info of all open dbf/index files (details see function)
   ...
   IF .T. //LEN(aOtables) <> 0
      ...
      IF lStandalone .or. lSimul_PlugIn_mode
         DEFINE WINDOW form_otis ;
            ...
            ;// Open Inspector IMMEDIATELY if there is any table opened at startup of Otis
      ...
      if len(aSelected) <> 0
         ...
         for i := 1 to len(aSelected)
            ...
            *msgstop(cAlias)
            ...
            // add always the area counter to the alias name
            // if is possible that you open 2 files with the same name thus the same alias
...
static function otis_add_index(oBrowse)
   ...
   local cOrdBagext, nOldsel, aSelected := {}, cTablename
      ...
      if DBORDERINFO(DBOI_BAGCOUNT) <> 0
         ...
         define window form_del_index ;
            ...
            DEFINE LABEL bt_Delete
               ...
               VCENTERALIGN .T.
               CENTERALIGN .T.
               ACTION { || otis_rem_index2(aIndexInfo), form_del_index.Release }
...
static function otis_rem_index2( aIndexInfo )

   local temp, i, fn_index, ds_name, cAlias
...
static function otis_get_area_info()
   ...
   local i, x, cAlias, aTblInfo, nStart, nEnd
...
static function Otis_settings()
   ...
   DEFINE WINDOW f_settings ;
      row getproperty("form_otis", "row") + 150 ;
      col getproperty("form_otis", "col") + 200 ;
      clientarea 425, 390 ;
      TITLE 'Otis - Settings' ;
      WINDOWTYPE MODAL ;
...
static function otis_about()
   ...
   MsgInfo("OTIS - Open Table InSpector (c)" + crlf + ;
             ...
             "This program is written with Harbour MiniGui and is FreeWare." + crlf + crlf + ;
...
static function table_prop(cWinname)
   ...
   local i, r, c, nOldsel, nNewsel, cAlias, aColVis, aItem, cTablename
   local aGrsize := {}
...
static function table_export( cTablename, cAlias, aColVis, cType )
   ...
   local i, temp, fqfn_fdest, filenamename, nOldsel ,;
         body         := "", ;
         header       := "", ;
         cClipboard   := "", ;
         prg_index_code, key, for
...
static function collect_index_info(cAlias)
   ...
   local i, x, temp, nOldsel, nOldorder
   local cInfo := "", prg_code := "", aIndexInfo := {}
   local nIndexFilesCount, nOrdercnt, cBagName
...
static function dv_viewer(nSelect)
   ...
   Local i, x, temp, r, c, gap
   Local cFontname, nFontsize
   Local cAlias, size, size1
   ...
   define window &cDvWinname ;
      AT GetDesktopHeight()/2 - int(GetDesktopHeight() * 0.75)/2, GetDesktopWidth()/2 - int(GetDesktopWidth() * 0.65)/2 ;
      clientarea int(GetDesktopWidth() * 0.80), int(GetDesktopHeight() * 0.75) ;
      ...
      * small empty label right of checkbox to obtain a visual centered checkbox
      ...
      DEFINE TBROWSE tb_Dv_Browse  ;
         ...
         // Footer settings
         //    index key count
         :aColumns[1]:cFooting := { |nc| hb_ntos((cAlias)->(Sx_KeyCount())) }
         //    footer :  reccount()
         :aColumns[3]:cFooting := { |nc| hb_ntos((cAlias)->(reccount())) }
...
// change set deleted status to show or not deleted() records.
   ...
   //ON LOSTFOCUS ThisWindow.release
      ...
      *
      ...
      DEFINE label lb_tofilter
         ...
         //CENTERALIGN .T.
         VISIBLE .T.
         ACTION { || dv_seek_to_filter(cWinname, cKey, aFields, aColVis, getproperty(ThisWindow.name, "lb_exact","Cargo")) }
         FONTCOLOR th_bt_fontcol
         BACKCOLOR th_bt_bgcol
         ONMOUSEHOVER { || setproperty(ThisWindow.name, This.name, "BACKCOLOR", th_bt_ohbgcol),;
                           setproperty(ThisWindow.name, This.name, "FONTCOLOR", th_bt_ohfontcol)}
...
static function dv_seek_to_filter(cWinname, cKey, aFields, aColVis, lExact)
...
static function dv_scope(cWinname, cAction)
   ...
   LOCAL temp, i, r, c, c1, lRefresh := .T.
...
static function dv_ai_rec(cWinname, cAction)
...
static function dv_dr_2( cAction, cWinname, cFor, cWhile, nScope, nNextRecords )
  ...
  LOCAL cFtype, bExpr, nRecno, nOldsel, lRest
  LOCAL lOk := .T.
...
static function dv_save(cWinname)
   ...
   local i, cFilename, lOk := .T., cFieldList := "All"
...
Static function Struct_editor()
  ...
  DEFINE WINDOW frmTableNew;
      AT 100, 100 ;
      CLIENTAREA 320 + th_bt_width + th_w_ctrlgap * 2, 600;
  ...
  IF !_HMG_DialogCancelled .AND. Len( aColVis ) > 0 .AND. !Empty( aColVis[1][1] )
    ...
    AEval( aColVis, { |a, i| aColVis[i][2] := iif( Left( a[2], 1 ) == "A", "+", Left( a[2], 1 ) ) } )

    // create dbf
    DBCreate( cFileDbf, aColVis, rddSetDefault() )
...
static function se_aim_field( nMode, cFormName )
  ...
  // Action code block for Confirm button
  local bConfirm := { ||
                                 ...
                                 // ok, len < 10
                                    ...
                                    // no invalid chars
                                       ...
                                       // store field def in tbrowse array
                                          ...
                                          cType2 := hb_HGetDef( aHTypes, iif( cType == "A", "+", cType ), 'C' )
  ...
  DEFINE WINDOW FieldNew;
      ...
      CLIENTAREA 335 + th_w_ctrlgap * 2 ,100 +th_w_ctrlgap * 2 ;
...
// set value and enable / disable some controls in function of type of field
...
static function se_load_struct()

   local cFn, aColVis, nOldsel, cRdd, oBrw
...
static function se_clear_struct()
   ...
   if MsgYesNo( "Do you want to clear the structure ?" )
      ...
      // clear textbox with name of loaded structure
...
static function leto_otis_getitems()
   ...
   local temp, i, aValues, aFiles := {}, nLen
...
//
****************************************************************
static function Leto_getfiles(cPath, cMask, TreeItem, cCallId)
****************************************************************
   ...
   // debug
   // msgstop(cPath + crlf + cMask + crlf + hb_ntos(TreeItem) + crlf + cCallId)
