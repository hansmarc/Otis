...
#define def_ini_lockscheme          DB_DBFLOCK_HB32
...
// used in combobox and others parts
Static aTypes := { "Character", ;
                   "Numeric", ;
                   "Date", ;
                   "Logical", ;
                   "Memo", ;
...
// field type code
Static aCTypes := { "C","N","D","L","M","D","D","M","+","=","^","T","@","I","I","I","I","I","V","V","V","V","Y","B"}
// field len for each code
Static aFldlen := {  0,  0,  8,  1, 10,  3,  4,  4,  4,  8,  8,  4,  8,  1,  2,  3,  4,  8,  3,  4,  6,  0,  8,  8 }
...
//******************************************************************************
   ...
   // locking scheme
   Public ini_lockscheme := DB_DBFLOCK_HB32
      ...
      *
      ...
      DEFINE LABEL bt_Defaults
         ...
         ACTION { || setproperty(ThisWindow.name, "tb_AreaDS"  ,"Value", val(def_Otisdb_area_nr) ), ;
                     ...
                     setproperty(ThisWindow.name, "tb_lockscheme","Value", def_ini_lockscheme) ;
                }
         // font and background color when onhover / onleave
         ONMOUSEHOVER { || setproperty( ThisWindow.name, This.name, "BACKCOLOR", th_bt_ohbgcol ),;
                           setproperty( ThisWindow.name, This.name, "FONTCOLOR", th_bt_ohfontcol ) }
...
static function se_aim_field( nMode, cFormName )
                                 ...
                                 // ok, len < 10
                                    ...
                                    // no invalid chars
                                       ...
                                       // store field def in tbrowse array
                                          ...
                                          cType2 := aCTypes[nType] + " - " + aTypes[nType]
  ...
  if nMode <> se_modify
   ...
   // if modify init with current values
      ...
      //nType   := AScan( aTypes, cType )
      nType   := AScan( aCTypes, left(cType,1) )
      IF Empty( nType )
         nType := 1
      ENDIF
   endif
  ...
  DEFINE WINDOW FieldNew;
      ...
      DEFINE COMBOBOX cmbTyp
          ...
          VALUE     nType
          ONCHANGE  ( lChanges := .T., ;
                      cType := aCtypes[This.Value], nType := This.Value, ;
                      se_OnTypeChange( This.Value, ThisWindow.Name, aFldlen ) )
      END COMBOBOX
  ...
  FieldNew.edtLen.Enabled := aFldlen[nType] == 0
...
//    and init field len.
static function se_OnTypeChange( nType, cWin, aFldlen )

   local nLen

   // enable/disable field len, variable if len=0
   SetProperty( cWin, "edtLen", "Enabled", aFldlen[nType] == 0 )

   // enable/disable field dec
   SetProperty( cWin, "edtDec", "Enabled", ( nType == 2 ) )

   // init len with default value from len array
   nLen := aFldlen[nType]
...
static function se_load_struct(cFormName)
   ...
   local i, cFn, aStruct, nOldsel, cRdd
   LOCAL oBrw := GetBrowseObj( "BRW_2", cFormName )
   ...
   if !empty(cFn)
      ...
      Try
         ...
         // translate type of field to a full type description
         //  ex C -> C - Character
         for i := 1 to len(aStruct)
            aStruct[i,2] := aStruct[i,2] + " - " + aTypes[ ascan(aCTypes,aStruct[i,2] ) ]
         next i
